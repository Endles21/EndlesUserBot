...
async def upstream(ups):
    ...
    await ups.edit("`Güncellemeler denetleniyor...EndlesUserBot`")
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head('master', origin.refs.endles)
        repo.heads.endles.set_tracking_branch(origin.refs.sql)
        repo.heads.endles.checkout(True)

    ac_br = repo.active_branch.name
    if ac_br != 'master':
        await ups.edit(
            f'**[Güncelleyici]:**` Galiba Endles botunu modifiye ettin ve kendi branşını kullanıyorsun: ({ac_br}). '
            'Bu durum güncelleyicinin kafasını karıştırıyor,'
            'Güncelleme nereden çekilecek?'
            'Lütfen Endles botunu resmi repodan kullan.`')
        repo.__del__()
        return
    ...
    if not changelog and not force_update:
        await ups.edit(
            f'\n`Botun` **tamamen güncel!** `ENDLES:` **{ac_br}**\n')
        repo.__del__()
        return
    ...
    if conf != "now" and not force_update:
        ...
        await ups.respond('`Güncellemeyi yapmak için \".update now\" komutunu kullan.\"Güncelleme yaklaşık 10 15 dakika arası sürer."\"EndlesUserBot.`')
        return
    ...
    if HEROKU_APIKEY is not None:
        ...
        await ups.edit('`[HEROKU MEMEZ]\
                        \nEndlesUserBot Heroku dynosuna aktarılıyor, lütfen bekle...`'
                       )
        ups_rem.fetch(ac_br)
        ...
        await ups.edit('`Güncelleme başarıyla tamamlandı!EndlesUserBot\n'
                       'EndlesUserBot Yeniden başlıyor...`')
