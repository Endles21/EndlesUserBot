...
async def ban(bon):
    ...
    if user.id in BRAIN_CHECKER:
        await bon.edit(
            "`Ban Hatası! Endles Yetkilisini yasaklayamam.`"
        )
        return

    # Hedefi yasaklayacağınızı duyurun
    await bon.edit("`Düşman Endles Tarafından vuruldu!`")
...
async def nothanos(unbon):
    ...
    await unbon.edit("`EndlesBot Yasağı Kaldırıyor Lütfen Bekleyiniz...`")
...
async def spider(spdr):
    ...
    if user.id in BRAIN_CHECKER:
        await spdr.edit(
            "`Mute Hatası! Endles Yetkilisini susturamam.`"
        )
        return
    ...
    await spdr.edit("`EndlesBot Kullanıcıyı Susturuken Lütfen Bekleyin...`")
...
async def gspider(gspdr):
    ...
    # Eğer kullanıcı sudo ise
    if user.id in BRAIN_CHECKER:
        await gspdr.edit("`Gmute Hatası! Endles Yetkilisini küresel olarak susturamam.`")
        return

    # Başarı olursa bilgi ver
    await gspdr.edit("`EndlesBot Kullacıyı Küresel Susturuyor...`")
...
async def rm_deletedacc(show):
    ...
    if con != "clean":
        await show.edit("`EndlesBot hayalet / silinmiş / zombi hesapları aranıyor...`")
    ...
    await show.edit("`EndlesBot Silinmiş hesapları çıkartıyor...`")
    del_u = 0
    del_a = 0
...
async def pin(msg):
    ...
    await msg.edit("`EndlesBot Mesajı Başarıyla sabitledi!`")
...
async def kick(usr):
    ...
    if user.id in BRAIN_CHECKER:
        await usr.edit(
            "`Kick Hatası! Endles Yetkilisini gruptan tekmeleyemem.`"
        )
        return

    await usr.edit("`EndlesBot Trafından Kullanıcı Çıkartılıyor...`")
