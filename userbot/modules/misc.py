...
async def randomise(items):
    ...
    if len(itemo) < 2:
        await items.edit(
            "`2 veya daha fazla eşya gerekli. Daha fazla bilgi için .endles random komutunu gir.`"
        )
        return
...
async def sleepybot(time):
    """ .sleep komutu Endles nin birkaç saniye uyumasına olanak sağlar. """
...
async def restart(event):
    await event.edit("`EndlesBot yeniden başlıyor...`")
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "#RESTART \n"
                                        "EndlesBot yeniden başladı.")
...
async def bot_support(wannahelp):
    ...
    await wannahelp.edit("[Buradan](http://t.me/EndlesSupport) destek grubumuza ulaşabilirsiniz.")
...
async def reedme(e):
    await e.edit("[EndlesBot README.md](https://github.com/Endles21/EndlesUserBot/blob/endles/README.md)")
...
async def repo_is_here(wannasee):
    ...
    await wannasee.edit("[EndlesBot Repo](https://github.com/Endles21/EndlesUserBot)")
...
CMD_HELP.update({
    'sleep':
    '.sleep <saniye>\
\nKullanım: Endles de bir insan, o da yoruluyor. Ara sıra biraz uyumasına izin ver.'
})
...
CMD_HELP.update({
    'repo':
    '.repo\
\nKullanım: Endles UserBot GitHub reposu'
})

CMD_HELP.update({
    "readme":
    ".readme\
\nKullanım: Endles botunun GitHub'daki README.md dosyasına giden bir bağlantı."
})
