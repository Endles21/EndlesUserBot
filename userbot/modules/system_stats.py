...
async def bot_ver(event):
    ...
    if which("git") is not None:
        ...
        await event.edit("`EndlesUserBot Versiyonu: "
                         f"{verout}"
                         "` \n"
                         "`Toplam değişiklik: "
                         f"{revout}"
        ...
        await event.edit(
            "Bu arada Endles seni çok seviyor. ❤"
        )
...
async def pipcheck(pip):
    ...
    if pipmodule:
        ...
        await pip.edit("`Bir örnek görmek için .endles pip komutunu kullanın.`")
...
CMD_HELP.update({
    "alive": ".alive\
    \nKullanım: Endles botunun çalışıp çalışmadığını kontrol etmek için kullanılır."
})
