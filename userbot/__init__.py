...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/Endles21/EndlesUserBot.git")
...
with bot:
    try:
        bot(JoinChannelRequest("@EndlesSupport"))
        bot(JoinChannelRequest("@EndlesUserBot"))
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@EndlesUserBot":
                rev_text = query[::-1]
                buttons = paginate_help(0, moduller, "helpme")
                result = builder.article(
                    f"Lütfen Sadece .yardım Komutu İle Kullanın",
                    text="{}\nYüklenen Modül Sayısı: {}".format(
                        "Merhaba! Ben @EndlesUserBot kullanıyorum!\n\nhttps://github.com/Endles21/EndlesUserBot", len(moduller)),
                    buttons=buttons,
                    link_preview=False
                )
                ...
                result = builder.article(
                    "© @EndlesUserBot",
                    text=f"@EndlesUserBot ile güçlendirildi",
                    buttons=[],
                    link_preview=True
                )
            else:
                result = builder.article(
                    "© @EndlesUserBot",
                    text="""@EndlesUserBot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/EndlesUserBot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/EndlesSupport")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/Endles21/EndlesUserBot")]
                    ],
                    link_preview=False
                )
            ...
            if event.query.user_id == uid:  # pylint:disable=E0602
                ...
                if len(cmdhel) > 90:
                    help_string = str(CMD_HELP[modul_name])[
                        :90] + "\n\nDevamı için .endles " + modul_name + " yazın."
                else:
                    help_string = str(CMD_HELP[modul_name])
